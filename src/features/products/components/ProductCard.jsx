...
const ProductCard = ({ item, onEdit, onDelete, onStockChange }) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-surface border-2 rounded-2xl p-4 transition-all hover:border-accent/30 ${
        isLowStock ? 'border-danger/30 bg-danger/5' : 'border-border'
      }`}
    >
      <div className="flex items-start gap-4">
        {/* Icon / Image */}
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
          item.isProduct ? 'bg-accent/10' : 'bg-gold/10'
        }`}>
          {item.imageUrl ? (
      ...
      {item.isProduct && (
        <div className="mt-3 pt-3 border-t border-border flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-lg bg-bg border border-border text-muted hover:text-danger hover:border-danger transition-colors"
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-bg border border-border text-muted hover:text-success hover:border-success transition-colors"
          ...
          <button
            onClick={() => onEdit?.(item)}
            className="p-1.5 rounded-lg bg-bg border border-border text-muted hover:text-white hover:border-white transition-colors"
            title="Editar"
          >
            <Edit size={14} />
          </button>
